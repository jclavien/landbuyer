...
let worker = setInterval(() => {
  ...
  connection.account.get(options.activeAccount, response => {
	...
	*/  
	...
	//// -------------------------------------------------------------------- Lancement pour la 2eme monnaie ------------------------------------------------------------
	if (account.pendingOrderCount > 0) {
      for (let order of account.orders) {
        if (order.type == 'TAKE_PROFIT' && order.instrument == ccyPair2) {
          takeProfitOrders2.push(round(order.price))
        } else if (order.type == 'MARKET_IF_TOUCHED' && order.instrument == ccyPair2) {
          limitOrders2.push(round(order.price))
        }
      }
      
      // On trie nos tableau dans l'ordre croissant
      takeProfitOrders2.sort()
      limitOrders2.sort()
    ...
    /*
      console.log('----')
      console.log('TAKE PROFIT ORDERS')
      console.log(takeProfitOrders2)
      console.log('LIMIT ORDERS')
      console.log(limitOrders2)
	*/  
      
      let highTradeValue2 = round(Math.max(...takeProfitOrders2) - distOnTakeProfit2)
      let lowTradeValue2 = round(Math.min(...takeProfitOrders2) - distOnTakeProfit2)

      console.log('----')
	  console.log(ccyPair2)
      console.log(`high trade: ${highTradeValue2}`)
      console.log(`low trade: ${lowTradeValue2}`)

      // On rempli un tableau avec les niveau de prix des ordres que l'on devrait ouvrir
      for (let i = 1; i < nbOrders; i++) { 
        searchedOrder = round(i * distBetweenPosition2 / 100 + highTradeValue2)

        if (!limitOrders2.includes(searchedOrder)) {
          ordersToBePlaced2.push(searchedOrder)
          takeProfitToBePlaced2.push (round(searchedOrder + distOnTakeProfit2))
        }
      }

       // Idem pour les ordres inférieurs
      for (let i = 1; i < nbOrders; i++) {
        searchedOrder= round(lowTradeValue2 - i * distBetweenPosition2 / 100)
      ...
      ordersToBePlaced2.sort()
      takeProfitToBePlaced2.sort()
	  console.log(ordersToBePlaced2)
      ...
      console.log('----')
      console.log('TAKE PROFIT TO BE PLACED')
      console.log(takeProfitToBePlaced2)
      console.log('----')
      */

      for (let i in ordersToBePlaced2) {
        // Ici on créer un objet de type LimitOrderRequest, avec différentes propriétés
        let order = new connection.order.MarketIfTouchedOrderRequest({
          instrument: ccyPair2,
          type: 'MARKET_IF_TOUCHED',
          units: positionAmount2,
          timeInForce: 'GTC',
          price: ordersToBePlaced2[i].toString(),
          takeProfitOnFill: {
            timeInForce: 'GTC',
            price: round(ordersToBePlaced2[i] + distOnTakeProfit2).toString()
          } 
        })
